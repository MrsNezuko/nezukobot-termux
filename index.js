...
const { help } = require('./NAUFAL/help')
const { rules } = require('./NAUFAL/rules')
const { bahasa } = require('./NAUFAL/bahasa')
const { donasi } = require('./NAUFAL/donasi')
const { limits } = require('./NAUFAL/limit')
const { bokep } = require('./NAUFAL/bokep.js')
const { ownermenu } = require('./NAUFAL/ownermenu.js')
const { groupmenu } = require('./NAUFAL/groupmenu.js')
const { mediamenu } = require('./NAUFAL/mediamenu.js')
const { nsfwmenu } = require('./NAUFAL/nsfwmenu.js')
const { listmenu } = require('./NAUFAL/listmenu.js')
const { funmenu } = require('./NAUFAL/funmenu.js')
const { makermenu } = require('./NAUFAL/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./NAUFAL/gabutmenu')
//const { ownermenu } = require('./NAUFAL/ownermenu')
//const { groupmenu } = require('./NAUFAL/groupmenu')
//const { vipmenu } = require('./NAUFAL/vipmenu')
//const { nsfwmenu } = require('./NAUFAL/nsfwmenu')
//const { creatormenu } = require('./NAUFAL/creatormenu')
//const { downloadmenu } = require('./NAUFAL/downloadmenu')
//const { allmenu } = require('./NAUFAL/allmenu')
...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:1.0\n'
            + 'FN:Naufal Admin🗿✨\n'//GANTI NAMA LU COK
            + 'ORG:Creator NEZUKOBOT;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=6285265938750:+62 852-6593-8750\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('BURUAN SCAN!! Subscribe Naufal Gamersss'))
	})

	fs.existsSync('./Fadhil.json') && client.loadAuthInfo('./Fadhil.json')
	client.on('connecting', () => {
		start('2', 'Putriii Chan...')
	})
	client.on('open', () => {
		success('2', 'Putriii Doi Gua!...')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["6285265938750@s.whatsapp.net"]
			...
			const ownerNumber = [
			"6285265938750@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"6285265938750@s.whatsapp.net","6285265938750@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'join':
			...
			case 'brainly':
					...
					teks = '❉─┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7❉\n'
					for (let Y of res.data) {
						teks += `\n*〄1�7 _BRAINLY_ 〄1�7*\n\n*➄1�7 Pertanyaan:* ${Y.pertanyaan}\n\n*➄1�7 Jawaban:* ${Y.jawaban[0].text}\n❉─┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7┄1�7❉\n`
					}
		...
		case 'antilink':
					...
					if (args[0] == 'on') {
						...
						fs.writeFileSync('./src/antilink.json', JSON.stringify(anlink))
						reply('Sukes menonaktifkan mode antilink︄1�7')
					} else {
			...
			case 'chatlist':
				...
				case 'totaluser':
					...
					if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `╭─┄1�7┄1�7┄1�7〄1�7 *TOTAL USER ${name}* 」\n`
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `[${no.toString()}] @${hehehe.split('@')[0]}\n`
					}
					teks += `┄1�7+ Total Pengguna : ${user.length}\n╰─┄1�7┄1�7┄1�7┄1�7┄1�7*⎄1�7 *${name}* ⏄1�7*┄1�7┄1�7┄1�7┄1�7`
		...
		case 'daftar':
			...
			case 'naufal':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {*/
					client.sendMessage(from, 'Terimakasih sudah menggunakan *NEZUKOBOT* \n\n*YUK SUPPORT*\nSubscribe https://youtube.com/c/NaufalGamersss \nFollow instagram https://instagram.com/ff_ulinkt', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'addprem':
					...
					addpremium = addpremi
					reply(`*ʙᴇʄ1�7ʜᴄ1�7ꜱɪʄ1�7 ᴍᴇɴᴄ1�7ᴍʙᴀʜᴋᴀɴ ${addpremium} ᴋᴇ ᴜꜱᴇʄ1�7 ᴩʄ1�7ᴇᴍɪᴜᴍ*\n\nꜱᴇʟᴄ1�7ᴍᴀᴄ1�7 ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ꜰɪᴛᴜʄ1�7 ᴩʄ1�7ᴇᴍɪᴜᴍ:)`)
					break
				case 'removeprem':
				case 'dellprem':
					if (!isOwner) return reply(mess.only.ownerB)
         ...
         case 'lirik':
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				case 'hentai':
			    	...
			    	if (!isUser) return reply(mess.only.userB)
					if (!isPrem) return reply(mess.only.premium)
						if (!isNsfw) return reply('❄1�7 *NSFW MATI* ❄1�7')
               ...
               if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   ...
                   reply(mess.wait)
                   gempa = `•Lokasi *${anu.lokasi}*\n 1�7 Saran: *${anu.waktu}* \n 1�7 Potensi: *${anu.potensi}*\n 1�7 Magnitude: *${anu.magnitude}*\n 1�7 Kedalaman: *${anu.kedalaman}*\n 1�7 Koordinat: *${anu.koordinat}*`
...
// akhir fitur anime
               ...
               case 'cersex':
                ...
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   ...
                   reply (mess.wait)
                   cerita = ` 1�7 *Judul:* ${anu.result.judul}\n\n${anu.result.cerita}`
			   ...
			   reply(mess.wait)
                    ...
                    teks = `===============\n`
                    for (let bokep of anu.result) {
                    teks += ` 1�7 Title: ${bokep.data.title}\n 1�7 Durasi: ${bokep.data.durasi}\n 1�7 Link: ${bokep.data.url}\n===============\n`
                    }
                    reply(teks.trim())
			     	await limitAdd(sender) 
			     	break 
			...
			case 'hekerbucin':
				...
				anu = await fetchJson(`https://api.vhtear.com/downloadfilm?judul=${body.slice(6)}&apikey=${VthearApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					film = ` 1�7 Judul: *${anu.result.judul}*\n 1�7 Resolusi: *${anu.result.resolusi}*\n 1�7 Link Download: *${anu.result.urlDownload}*\n`
...
//akhir kreator
			   ...
			   case 'igstalk':
                    if (isBanned) return reply(mess.only.benned)    
                     ...
                     reply(mess.wait)
                     hasil = `〄1�7 *INSTAGRAM STALKER* 」\n\n 1�7 Link: https://www.instagram.com/${anu.Username}\n 1�7 Fullname : ${anu.Name}\n 1�7 Following : ${anu.Jumlah_Followers}\n 1�7 Followers : ${anu.Jumlah_Following}\n 1�7 Jumlah Postingan: ${anu.Jumlah_Post}\n 1�7 Bio : ${anu.Biodata}`
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✄1�7 Tag By *${pushname2}* ✪══`+ teks +'╚═══〄1�7 ${name} 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══�1�7�1�7  *${body.slice(12)}*  〙✪══`+teks+'╚═〄1�7 BRYANBOT 〄1�7', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, bcgc, image, {caption: `*〄1�7 BROADCAST GROUP BY ${pushname2} 〄1�7*\n\n*Group* : ${groupName}\n*Pada Jam* :${jam}\n\n${name}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*〄1�7 BROADCAST GROUP BY ${pushname2} 〄1�7*\n\n*Group* : ${groupName}\n${name}\n*Pada Jam*: ${jam}WIB\n\n${body.slice(6)}`)
						}
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, bc, image, {caption: `[ PUN10 BROADCAST ]\n\n${body.slice(4)}`})
						}
						reply('Suksess broadcast')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ *NEZUKOBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					setTimeout( () => {
					client.sendMessage(from, '*➄1�7 Jawaban :* '+anu.result.jawaban+ '\n\n 1�7 Penjelasan: *'+ anu.result.desk+'*', text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_20 Detik lagi_ 1�7', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_30 Detik lagi_ 1�7', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					setTimeout( () => {
					client.sendMessage(from, '*➄1�7 Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`Prefix berhasil di ubah menjadi :〄1�7* ${prefix} *」`)
					break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	case 'infocuaca':
     ...
     reply(mess.wait)
            ...
            if (weather.error) {
             ...
             client.sendMessage(from, `➄1�7 Tempat : ${weather.result.tempat}\n\n➄1�7 Angin : ${weather.result.angin}\n➄1�7 Cuaca : ${weather.result.cuaca}\n➄1�7 Deskripsi : ${weather.result.desc}\n➄1�7 Kelembapan : ${weather.result.kelembapan}\n➄1�7 Suhu : ${weather.result.suhu}\n➄1�7 Udara : ${weather.result.udara}`, text, {quoted: mek})
            }
            await limitAdd(sender)
            break 
		...
		case 'jadwalsholat':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					jsol = `Jadwal sholat di *${sholat}* hari ini adalah\n\n➄1�7 *Subuh :* ${anu.Subuh} WIB\n*➄1�7 Dzuhur :* ${anu.Dzuhur} WIB\n*➄1�7 Ashar :* ${anu.Ashar} WIB\n*➄1�7 Maghrib :* ${anu.Maghrib} WIB\n*➄1�7 Isya :* ${anu.Isya} WIB`
...
// premium user
         case 'joox':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `〄1�7 *JOOX* 」\n\n* 1�7 Judul* : ${anu.result.judul}\n* 1�7 Album* : ${anu.result.album}\n* 1�7 Dipublikasi* : ${anu.result.dipublikasi}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM*`
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                 sck = `〄1�7 *SNACK VIDEO DOWNLOADER* 」\n\n* 1�7 Format:* ${anu.format}\n* 1�7 Size:* ${anu.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM*`
             ...
             case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					ytt = `〄1�7 *YOUTUBE MP4 DOWNLOADER* 」\n\n 1�7 Title : *${anu.title}*\n 1�7 *Size:* ${anu.filesize}\n 1�7 *Deskripsi:* ${anu.desc}\n\n Tunggu Sebentar 1 menit Mungkin Agak Lama Karna Mendownload Video`
				...
				case 'ytmp3':
					...
					if (anu.error) return reply(anu.error)
					teks = `〄1�7 *YOUTUBE MP3 DOWNLOADER* 」\n\n 1�7 Title : *${anu.title}*\n 1�7 *Size:* ${anu.filesize}\n* 1�7 Deskripsi:* ${anu.desc}\n\n Tunggu Sebentar 1 menit Mungkin Agak Lama Karna Mendownload Video`
					buff = await getBuffer(anu.thumb)
					reply(mess.wait)
					buffer = await getBuffer(anu.result)
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `〄1�7 *PLAY MP3* 」\n* 1�7 Judul:* ${anu.result.title}\n* 1�7 Source:* ${anu.result.source}\n* 1�7 Ukuran:* ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA SAYANG*`
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[NEZUKOBOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
